(function () {
  ...
  window.CollisionManager = function (engine) {
    ...
    Matter.Events.on(engine, 'collisionEnd', function(event) {
      ...
      for (var i = 0, j = pairs.length; i != j; ++i) {
        ...
        for(var k = 0; k < game.objectsToRender.length; k++) {
          if(game.objectsToRender[k].physics === pair.bodyA){
            var gameObject = game.objectsToRender[k];
            if(gameObject.hit){
              gameObject.hit(pair.bodyB);
            }
          }
          if(game.objectsToRender[k].physics === pair.bodyB){
            var gameObject = game.objectsToRender[k];
            if(gameObject.hit){
